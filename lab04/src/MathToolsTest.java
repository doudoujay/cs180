...
/**
 * Created by jay on 2/7/17.
 */
public class MathToolsTest {
    private final double DELTA = 0.000001;

    @Test(timeout = 100)
    public void testNthRootSmallNumber(){
        double actual = MathTools.nthRoot(4.0,2);
        double expected = 2.0;
        assertEquals("Check Small number",expected,actual,DELTA);
    }
    @Test(timeout = 100)
    public void testNthRootMiddleNumber(){
        double actual = MathTools.nthRoot(100,10);
        double expected = 2.0;
        assertEquals("Check Small number",expected,actual,DELTA);
    }
    @Test(timeout = 100)
    public void testNthRootThreeNumber(){
        double actual = MathTools.nthRoot(9,3);
        double expected = 2.0;
        assertEquals("Check Small number",expected,actual,DELTA);
    }
